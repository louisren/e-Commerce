sap.ui.define(
	...
	function (MockServer: typeof sap.ui.core.util.MockServer, Log: any) {
		...
		class TsAppMockServer {
			 ...
			 */
			init(): void {
				this._oLogger = Log.getLogger('bptfront.TsApp.mockserver')
				const aRequests = []
			...
			private createGetUserList(): Request {
				...
				return {
					...
					response: (xhr): void => {
						...
						//@ts-ignore
						let filteredUserList: Array<bptfront.TsApp.data.UserData> = that._userListModel.getProperty(
							'/users'
						)
						if (filteredUserList) {
			...
			private generateUHDUserData(
				userData: bptfront.TsApp.data.UserData
			): bptfront.TsApp.data.UHDData {
				const arrAppIds = ['fr2', 'bb4', 'fk5', 'hg3', 'ur9', 'zz0', 'zz2', 'zz3', 'op4', 'pu3']
				const arrRoleIds = ['role1', 'role2', 'role3', 'role4']
				const retValue: bptfront.TsApp.data.UHDData = {}
