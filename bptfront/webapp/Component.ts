sap.ui.define(
	[
		...
		'bptfront/TsApp/model/models',
		'sap/base/Log',
		'sap/ui/core/Popup',
		'bptfront/TsApp/controls/StripToaster',
	],
	function (
		...
		class Component extends UIComponent {
			...
			constructor(mSettings: object) {
				...
				const fnClass = UIComponent.extend('bptfront.TsApp.Component', {
					metadata: {
						manifest: 'json',
					}
				})
				...
				super('bptfront.TsApp.Component', mSettings)
			}
			...
			init(): void {
				...
				this._logger = Log.getLogger('bptfront.TsApp.Component')
				// enable routing
			 ...
			 * Die Daten des Models entsprechen der Struktur bptfront.SuperAdminApp.data.AppParamsModelData
			 *
			 ...
			 */
			public getUserList(input: string): Promise<Array<bptfront.TsApp.data.UserData>> {
					...
					xhr.onload = function (): void {
						if (xhr.status === 200) {
							...
							if (rawData && rawData.result && rawData.result === 'error') {
								...
								let data: Array<bptfront.TsApp.data.UserData> = JSON.parse(xhr.responseText)
			 ...
			 */
			public getUserData(userId: string): Promise<bptfront.TsApp.data.UHDData> {
				return new Promise((resolve, reject) => {
					const xhr = new XMLHttpRequest()
					...
					xhr.onload = function () {
						if (xhr.status === 200) {
							...
							if (rawData && rawData.result && rawData.result === 'error') {
								...
								const data = rawData as bptfront.TsApp.data.UHDData
