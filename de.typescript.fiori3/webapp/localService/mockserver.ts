sap.ui.define(
	...
	function (MockServer: typeof sap.ui.core.util.MockServer, Log: any) {
		'use strict'
		...
		class TsAppMockServer {
			_oLogger: any
			_prefix = 'de/typescript/fiori3/TsApp'
			 ...
			 */
			init(): void {
				this._oLogger = Log.getLogger('de.typescript.fiori3.TsApp.mockserver')
				const aRequests = []
			...
			private createGetUserList(): Request {
				...
				return {
					...
					response: (xhr): void => {
						...
						//@ts-ignore
						let filteredUserList: Array<de.typescript.fiori3.TsApp.data.UserData> = that._userListModel.getProperty(
							'/users'
						)
						if (filteredUserList) {
							filteredUserList = filteredUserList.filter((obj) => {
								return (
									obj.userId.indexOf(input) > -1 || obj.lastName.indexOf(input) > -1 || obj.email.indexOf(input) > -1
								)
							})
			 ...
			 * @param userData
			 */
			private generateUHDUserData(
				userData: de.typescript.fiori3.TsApp.data.UserData
			): de.typescript.fiori3.TsApp.data.UHDData {
				const arrAppIds = ['fr2', 'bb4', 'fk5', 'hg3', 'ur9', 'zz0', 'zz2', 'zz3', 'op4', 'pu3']
				const arrRoleIds = ['role1', 'role2', 'role3', 'role4']
				const retValue: de.typescript.fiori3.TsApp.data.UHDData = {}
