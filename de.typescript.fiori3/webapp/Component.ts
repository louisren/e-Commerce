sap.ui.define(
	...
	function (
		...
		class Component extends UIComponent {
			 ...
			 */
			public getUserList(input: string): Promise<Array<de.typescript.fiori3.TsApp.data.UserData>> {
					...
					xhr.onload = function (): void {
						if (xhr.status === 200) {
							...
							if (rawData && rawData.result && rawData.result === 'error') {
								...
								let data: Array<de.typescript.fiori3.TsApp.data.UserData> = JSON.parse(xhr.responseText)
			 ...
			 */
			public getUserData(userId: string): Promise<de.typescript.fiori3.TsApp.data.UHDData> {
				return new Promise((resolve, reject) => {
					const xhr = new XMLHttpRequest()
					...
					xhr.onload = function () {
						if (xhr.status === 200) {
							...
							if (rawData && rawData.result && rawData.result === 'error') {
								...
								const data = rawData as de.typescript.fiori3.TsApp.data.UHDData
