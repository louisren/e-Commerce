sap.ui.define(
	[
		...
		'pscctoolsfront/TsApp/model/models',
		'sap/base/Log',
		'sap/ui/core/Popup',
		'pscctoolsfront/TsApp/controls/StripToaster',
	],
	function (
		...
		class Component extends UIComponent {
			...
			constructor(mSettings: object) {
				...
				const fnClass = UIComponent.extend('pscctoolsfront.TsApp.Component', {
					metadata: {
						manifest: 'json',
					}
				})
				...
				super('pscctoolsfront.TsApp.Component', mSettings)
			}
			...
			init(): void {
				...
				this._logger = Log.getLogger('pscctoolsfront.TsApp.Component')
				// enable routing
			 ...
			 * Die Daten des Models entsprechen der Struktur pscctoolsfront.SuperAdminApp.data.AppParamsModelData
			 *
			 ...
			 */
			public getUserList(input: string): Promise<Array<pscctoolsfront.TsApp.data.UserData>> {
					...
					xhr.onload = function (): void {
						if (xhr.status === 200) {
							...
							if (rawData && rawData.result && rawData.result === 'error') {
								...
								let data: Array<pscctoolsfront.TsApp.data.UserData> = JSON.parse(xhr.responseText)
			 ...
			 */
			public getUserData(userId: string): Promise<pscctoolsfront.TsApp.data.UHDData> {
				return new Promise((resolve, reject) => {
					const xhr = new XMLHttpRequest()
					...
					xhr.onload = function () {
						if (xhr.status === 200) {
							...
							if (rawData && rawData.result && rawData.result === 'error') {
								...
								const data = rawData as pscctoolsfront.TsApp.data.UHDData
